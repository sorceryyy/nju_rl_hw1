...
class RNDAgent(object):
    ...
    def get_action(self, state):
        # if dim < 4, add batch on first dim
    ...
    def load(self, dir, env_id, use_cuda=True):
        model_path = os.path.join(dir, '{}.model'.format(env_id))
        predictor_path = os.path.join(dir, '{}.pred'.format(env_id))
        target_path = os.path.join(dir, '{}.target'.format(env_id))

        print('Loading Pre-trained model....')
        if use_cuda:
            self.model.load_state_dict(torch.load(model_path))
            self.rnd.predictor.load_state_dict(torch.load(predictor_path))
            self.rnd.target.load_state_dict(torch.load(target_path))
        else:
            self.model.load_state_dict(torch.load(model_path, map_location='cpu'))
            self.rnd.predictor.load_state_dict(torch.load(predictor_path, map_location='cpu'))
            self.rnd.target.load_state_dict(torch.load(target_path, map_location='cpu'))
        print('End load...')
